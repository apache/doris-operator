...
CLOUD_UNIQUE_ID=${CLOUD_UNIQUE_ID:="1:$INSTANCE_ID:$STATEFULSET_NAME"}
# replace "-" with "_" in CLUSTER_ID and CLOUD_UNIQUE_ID
CLUSTER_ID=$(sed 's/-/_/g' <<<$CLUSTER_ID)
CLOUD_UNIQUE_ID=$(sed 's/-/_/g' <<<$CLOUD_UNIQUE_ID)
...
{
    local key=$1
    local value=`grep "^\s*$key\s*=" $CONFIG_FILE | sed "s|^\s$key\s*=\(.*\)\s*|\1|g"`
}
